...
// Simple in-memory status for UI (not persistent across reboot)
String lastStatus = "Ready";
int lastBudget = -1;

// Flag to pause GATE mode during web handler operations
bool gateModePaused = false;
...
// Read current budget, add amountToAdd (capped at 9999), and write back - all in one session
bool readAddWriteBudget(int amountToAdd, int &newBudget) {
  if (!selectAndAuth(BUDGET_BLOCK)) return false;
  ...
  int currentBudget = 0;
  if (raw.startsWith("B=")) {
    currentBudget = raw.substring(2).toInt();
  }

  Serial.print("Read current budget = ");
  Serial.println(currentBudget);

  // Calculate new budget (capped at 9999)
  int target = currentBudget + amountToAdd;
  if (target > 9999) target = 9999;

  // Write new budget (same session, no re-auth needed)
  byte writeBuffer[16];
  memset(writeBuffer, 0, sizeof(writeBuffer));
  String s = "B=" + String(target);
  s.toCharArray((char*)writeBuffer, 16);

  status = mfrc522.MIFARE_Write(BUDGET_BLOCK, writeBuffer, 16);
  if (status != MFRC522::STATUS_OK) {
    Serial.print("Write failed: ");
    Serial.println(mfrc522.GetStatusCodeName(status));
    mfrc522.PICC_HaltA();
...
void handleControlPage() {
  ...
  String html = pageHeader("Control Panel");

  html += "<h3>Rav-Kav (NFC)</h3>";
  html += "<p>Status: <b>" + lastStatus + "</b></p>";
  if (lastBudget >= 0) {
    html += "<p>Last budget value: <b>" + String(lastBudget) + "</b></p>";
  }
  html += "<form method='GET' action='/ravkav/load'>";
  html += "Add to budget: <input name='budget' type='number' min='0' max='9999' required>";
  html += "<button type='submit'>Add to card</button>";
  html += "</form>";

  html += "<form method='GET' action='/ravkav/read'>";
  html += "<button type='submit'>Read budget from card</button>";
  html += "</form>";

  html += "<p style='font-size:12px;color:#555'>Tip: press the button, then present the card to the reader within ~5 seconds.</p>";
...
// ---- RavKav Web handlers (before LCD integration) ----
bool waitForLoadBudgetToCard(int amountToAdd, unsigned long timeoutMs, int &newBudget, String &msg) {
  unsigned long start = millis();
  while (millis() - start < timeoutMs) {
    int result;
    if (readAddWriteBudget(amountToAdd, result)) {
      newBudget = result;
      msg = "Added " + String(amountToAdd) + " to budget. New budget=" + String(result);
      return true;
    }
    delay(50);
  }
  msg = "Timeout: no card detected (present a card to the reader)";
  return false;
}

bool waitForGateDecrement(unsigned long timeoutMs, int &newBudget, String &msg) {
  unsigned long start = millis();
  while (millis() - start < timeoutMs) {
    bool ok = false;
    int b = readBudget(ok);
    if (ok) {
      if (b <= 0) {
        newBudget = 0;
        msg = "Denied: budget=0";
        return true; // gate processed, but denied
      }
      // try to write back decremented value (may need a short retry)
      int target = b - 1;
      unsigned long wstart = millis();
      while (millis() - wstart < 1000) {
        if (writeBudget(target)) {
          newBudget = target;
          msg = "Entry OK. New budget=" + String(target);
          return true;
        }
        delay(50);
      }
      msg = "Read OK but failed to write new budget. Try again.";
      return false;
    }
    delay(50);
  }
...
void redirectToControl() {
  server.sendHeader("Location", "/control");
  server.send(302, "text/plain", "Redirecting...");
}

void handleRavkavLoad() {
  if (!loggedIn) { server.send(403, "text/plain", "Forbidden"); return; }

  gateModePaused = true;  // Pause GATE mode during this operation

  if (!server.hasArg("budget")) {
    lastStatus = "Missing budget parameter";
    gateModePaused = false;
    redirectToControl();
    return;
  }

  int budget = server.arg("budget").toInt();
  if (budget < 0 || budget > 9999) {
    lastStatus = "Invalid budget (0..9999)";
    gateModePaused = false;
    redirectToControl();
    return;
  }
  ...
  String msg;
  int newBudget = -1;
  bool ok = waitForLoadBudgetToCard(budget, 5000, newBudget, msg);
  lastStatus = msg;
  if (ok && newBudget >= 0) lastBudget = newBudget;
  gateModePaused = false;  // Resume GATE mode
  redirectToControl();
}
...
void handleRavkavRead() {
  ...
  lastStatus = "Timeout: no card detected (present a card to the reader)";
  gateModePaused = false;  // Resume GATE mode
  redirectToControl();
}
...
void setup() {
  ...
  server.on("/ravkav/load", HTTP_GET, handleRavkavLoad);
  server.on("/ravkav/read", HTTP_GET, handleRavkavRead);
...
void loop() {
  ...
  // Skip GATE mode when web handlers are active
  if (gateModePaused) return;
